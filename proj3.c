...
void merge_clusters(struct cluster_t *c1, struct cluster_t *c2)
{
    ...
    int predchozi_c1_size = c1->size, i;
    for(i = 0; i < c2->size; i++){
        append_cluster(c1, c2->obj[i]);
    }
    if(c2->size > 0 && c1->size == predchozi_c1_size + c2->size){
        sort_cluster(c1);
    }
}
...
float cluster_distance(struct cluster_t *c1, struct cluster_t *c2)
{
    ...
    int i, j;
    float vzdalenost, konecna_vzdalenost = 0.0;
    for(i = 0; i < c1->size; i++){
        for(j = i; j < c2->size; j++){
            vzdalenost = obj_distance(&c1->obj[i], &c2->obj[j]);
            if(vzdalenost > konecna_vzdalenost){
                konecna_vzdalenost = vzdalenost;
            }
        }
    }
    return konecna_vzdalenost;
}
...
int load_clusters(char *filename, struct cluster_t **arr)
{
    ...
    int MAX_delka_radku = 1024;
    int pocet_objektu, i;
    char radek[MAX_delka_radku], *p;
    
    fgets(radek, MAX_delka_radku, file);
    pocet_objektu = strtol(radek + 6, &p, 10);
    if(pocet_objektu <= 0){
        printf("Pocet objektu musi byt vetsi nez 0.\n");
        fclose(file);
        return -1;
    }
    *arr = malloc(pocet_objektu * sizeof(struct cluster_t));
        ...
        append_cluster(&(*arr)[i], (struct obj_t){
            .id = strtol(radek, &p, 10),
            .x = strtof(p, &p),
            .y = strtof(p, NULL)
        });
    }
...
int main(int argc, char *argv[])
{
    ...
    char *p = NULL;
    int pocet_shluku = strtol(argv[2], &p, 10);
    if(argc == 3){
        if(*p){
            ...
            printf("Argument N musi byt vetsi cislo nez 0\n");
            printf("\nPro spusteni programu napiste: ./proj3 SOUBOR [N]   SOUBOR = jmeno souboru se vstupnimi daty  [N] = volitelny argument definujici cilovy pocet shluku N > 0.");
            return 1;
        }
        if(load_clusters(argv[1], &clusters) == -1) return 0;
