...
double taylor_log(double x, unsigned int n){
	double answer = 0.0, numerator = 1.0, fraction;
	int i;
...
int main(int argc, char *argv[]){
	...
	if(strcmp(argv[1], "--log")==0){
		if(argc != 4){
			...
			errno = 0;
			//checking for any errors, if there is a character or if x is too big
			if(errno != 0 || *p != '\0' || x>INT_MAX)
			{
				printf("Wrong input of arguments. The argument is not a number.\n");
				return 1;
			}
			printf("\tlog(%.4g) = %.12g\n", x, log(x));
